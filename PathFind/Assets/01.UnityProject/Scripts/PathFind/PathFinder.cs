...
public class PathFinder : GioleSingletone<PathFinder>
{
    #region 지형 탐색을 위한 변수
    ...
    private IEnumerator DelayFindPath_Astar(float delay_)
    {
        ...
        while (isFoundDestination == false && isNowayToGo == false)
        {
            ...
            else
            {
                ...
                // 이 떄, Open list 가 비어 있다면 더 이상 탐색 할 수 있는 길이
                // 존재하지 않는 것이다.
                aStarClosePath.Add(minCostNode);
                aStarOpenPath.Remove(minCostNode);
                if (aStarOpenPath.IsVaild() == false)
                {
                    GioleFunc.LogWarning("[Warning] There are no more tiles to explore.");
                    isNowayToGo = true;
                }       // if: 목적지에 도착하지 못했는데, 더 이상 탐색할 수 있는 길이 없는 경우

                foreach (var tempNode in aStarOpenPath)
                {
                    GioleFunc.Log($"Idx: {tempNode.Terrain.TileIdx1D}, " +
        ...
        AstarNode targetTerrain_, AstarNode prevNode = default)
    {
        ...
        else
        {
            ...
            }       // if: Open list 에 추가할 노드와 같은 좌표의 노드가 존재하는 경우
            else
            {
                aStarOpenPath.Add(targetTerrain_);
        ...
        AstarNode targetNode, AstarNode prevNode)
    {
        ...
        // { 이전 노드가 존재하는 경우 이전 노트의 코스트를 추가해서 연산한다.
